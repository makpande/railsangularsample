class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  def twitter
    ...
    if auth
      # We already know about this user who is signing in with the provider
      #then just return the user associated with the Authorization
      user = auth.user
    else
      ...
      unless current_user
        unless user = User.find_by_name(name)
          user = User.create({
            ...
            email: "#{UUIDTools::UUID.random_create}@maksshareup.com"
            })
        end
      else
        user = current_user
